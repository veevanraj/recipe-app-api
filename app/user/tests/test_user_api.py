...
class PublicUserApiTests(TestCase):
    ...
    def test_create_valid_user_success(self):
        """Test creating user with valid payload issuccessful"""
        payload = {
    ...
    def test_create_token_no_user(self):
        ...
        payload ={'email': 'test@londonaapdev.com', 'password': 'testpass'}
        res =self.client.post(TOKEN_URL, payload)
...
class PrivateUserApiTests(TestCase):
    ...
    """test api request that required api authentication"""
    def setUp(self):
    ...
    def test_retrieve_profile_success(self):
            """test retriving for logged in used """
            res = self.client.get(ME_URL)
            self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_post_me_not_allowed(self):
        """test that not allowed  on the me url"""
        res = self.client.post(ME_URL, {})

        self.assertEqual(res.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def test_update_user_profile(self):
        """test updating for user profile for authenticated user"""
